...
typedef actionlib::SimpleActionClient<aerialmanip_control::waypointsAction> Client;
...
class robotClient {
  public:
    robotClient();
    bool sendGoal(const aerialmanip_control::waypointsGoal& goal);
  ...
  private:
    ...
    void feedbackCb(const aerialmanip_control::waypointsFeedbackConstPtr& feedback);

    ros::NodeHandle _nh;
    ros::Subscriber _cartpose_sub, _extWrench_sub;
    actionlib::SimpleActionClient<aerialmanip_control::waypointsAction> _ac;
    aerialmanip_control::waypointsFeedback _feedback;
    aerialmanip_control::waypointsResult _result;
    aerialmanip_control::waypointsGoal _goal;
...
robotClient::robotClient() : _ac("armActionServer", true) {
  _firstpose=_firstwrench=false;
  ROS_INFO("Waiting for action server to start.");
  // wait for the action server to start
  _ac.waitForServer(); //will wait for infinite time
  ROS_INFO("Action server started.");
  _cartpose_sub = _nh.subscribe("/arm/eef_des_pose", 0, &robotClient::pose_cb, this);
  _extWrench_sub = _nh.subscribe("/arm/eef_ext_wrench", 0, &robotClient::wrench_cb, this);
  _completePerc=0;
}
...
void robotClient::feedbackCb(const aerialmanip_control::waypointsFeedbackConstPtr& feedback) {
  _completePerc = feedback->completePerc;
  //cout<<"Trajectory completed at "<<feedback->completePerc<<"% ."<<endl;
}

bool robotClient::sendGoal(const aerialmanip_control::waypointsGoal& goal) {
...
int main (int argc, char **argv)
{
  ...
  aerialmanip_control::waypointsGoal goal;
...
*/
  ...
  //waypoints.poses[1].pose.position.x += 0.10;
